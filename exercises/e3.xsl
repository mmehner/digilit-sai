...
<xsl:stylesheet
  ...
  <xsl:variable name="n">
   ...
   <xsl:variable name="wit">#D</xsl:variable>
  ...
  <xsl:template match="/">
    ...
    <xsl:value-of select="$n"/>
    <xsl:value-of select="$n"/>
    <xsl:text>Witness reference: </xsl:text>
    <xsl:value-of select="$wit"/>
    <xsl:value-of select="$n"/>
    <xsl:text>Variants:</xsl:text>
    <xsl:value-of select="$n"/>
    <!-- separate templates for more complex operations -->
    <!-- <xsl:apply-templates select="//t:app/*[@wit = $wit]"/> -->
    <xsl:apply-templates select="//t:app/*[contains(@wit, $wit)]"/>
  </xsl:template>
